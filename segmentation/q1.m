...
maskId = 88;
h_student = histvec(im,S==maskId,10);
h_correct = j_histvec(im,S==maskId,10);
...
figure()
bg = 0.1;
i1 = im(:,:,1).*(S==maskId) + bg*im(:,:,1).*(S~=maskId);
i2 = im(:,:,2).*(S==maskId) + bg*im(:,:,2).*(S~=maskId);
i3 = im(:,:,3).*(S==maskId) + bg*im(:,:,3).*(S~=maskId);
imagesc(cat(3,i1,i2,i3))